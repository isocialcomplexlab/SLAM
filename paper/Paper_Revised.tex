...
\begin{revisionblock}
Beyond global NetVLAD, Patch-NetVLAD derives patch-level features from NetVLAD residuals and fuses multiple patch scales to combine local and global place-recognition cues \cite{Hausler2021PatchNetVLAD}. Complementarily, the Tokyo 24/7 benchmark was introduced to evaluate large-scale place recognition under major appearance changes, including day, sunset, and night conditions, together with viewpoint differences \cite{Torii2015Tokyo247}. These works provide relevant context for condition- and viewpoint-robust Visual Place Recognition; they are discussed here as related evidence and were not included as additional experimental baselines in the present study.
\end{revisionblock}
...
% R2-M2 preprocessing-ablation clarification (candidate patch)
\begin{revisionblock}
To evaluate the effect of the historical left/right image-splitting strategy,
we additionally considered a full-image preprocessing mode. The two
configurations are treated as complete preprocessing pipelines because the
historical split mode concatenates descriptors from the two image halves,
whereas the full-image mode produces a single descriptor. Therefore, this
ablation evaluates the practical preprocessing pipeline rather than claiming
a perfectly isolated crop-only intervention. The historical splitting
strategy is motivated by Garg et al.~\cite{Garg2018DontLookBack}; its
empirical validity is assessed by the controlled ablation reported below.
\end{revisionblock}
...
\textcolor{red}{(c)}
\end{minipage}
\caption{\protect\revisioncaption{Representative image sequences from (a) KITTI Group 5, (b) KITTI Group 8, and (c) the evaluated TUM sequence. The three sequences are consolidated into one compact horizontal panel to reduce figure space while preserving the visual comparison.}}
...
% R2-M2 controlled preprocessing ablation (candidate patch)
For KITTI 05 and KITTI 08, the split-left/right and full-image preprocessing
modes were evaluated on identical pair identities. The corrected-L2 operating
threshold for each preprocessing mode was selected using that mode's
calibration split only and was then applied to the corresponding held-out test
split. Paired bootstrap confidence intervals were computed for the
full-image-minus-split differences in ROC-AUC and average precision. The
KITTI 08 stress subset contains no negative binary pairs and is therefore
limited to recall/sensitivity-compatible measures. TUM19 was evaluated with
threshold-free retrieval/spatial measures; these values are reported
separately and are not numerically pooled with the KITTI classification
metrics.

On TUM19, full-image processing reduced the mean top-1 spatial error from
0.189~m to 0.142~m and increased MRR from 0.020 to 0.056. On KITTI 05,
held-out ROC-AUC increased from 0.824 to 0.851 and average precision from
0.594 to 0.677; the paired bootstrap 95\% confidence intervals for the two
improvements were entirely above zero. KITTI 08 showed no comparable
held-out ranking advantage: ROC-AUC changed from 0.485 to 0.494 and average
precision from 0.052 to 0.050, with both paired bootstrap confidence
...
\begin{revisionblock}
In Group 5, the Adapted and Fine-Tuned VGG-16 achieved the best results for Loop Closure Detection within that evaluated sequence. In Group 8, ResNet achieved the best F1-score among the evaluated architectures. In this sequence, opposite traversal was associated with reduced descriptor consistency. Because the study did not perform a controlled internal-representation analysis for each architecture, these condition-specific differences should not be interpreted as evidence of a specific architectural cause.
\end{revisionblock}


\pagebreak[4]

To assess controlled cross-sequence transfer, we applied the fixed ResNet-101 descriptor pipeline to KITTI 08 using thresholds calibrated only on KITTI 05; no KITTI 08 recalibration was performed. On the KITTI 08 test split, legacy exact and corrected L2 obtained ROC-AUC values of 0.536 and 0.485, with frozen-threshold recall of 0.0055 and 0.0051, respectively (Table~\ref{tab:kitti08_cross_sequence_transfer}). Average precision values of 0.0569 and 0.0522 were near the 0.0524 positive prevalence, and the corrected-L2 advantage observed on KITTI 05 did not persist. The positive-only stress split yielded recall of 0.0165 and 0.0000 and is treated only as a sensitivity diagnostic. These results indicate a sequence/domain generalization limitation of the fixed descriptor-and-threshold pipeline without identifying its specific cause.

\begin{table*}[t]
\centering
\caption{Controlled cross-sequence transfer from KITTI 05 to KITTI 08 using fixed ResNet-101 descriptors. Thresholds calibrated on the KITTI 05 calibration split are frozen on KITTI 08; $\Delta$ denotes KITTI 08 minus KITTI 05. Ambiguous pairs are excluded from binary metrics, and the positive-only KITTI 08 stress split reports recall only.}
\label{tab:kitti08_cross_sequence_transfer}
\small
\renewcommand{\arraystretch}{1.12}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrrrrr@{}}
\hline
Method & \shortstack{KITTI 05\\test AUC} & \shortstack{KITTI 08\\test AUC} & $\Delta$AUC & \shortstack{KITTI 05\\test recall} & \shortstack{KITTI 08\\test recall} & $\Delta$recall & \shortstack{KITTI 08\\stress recall} \\
\hline
Legacy exact & 0.7305 & 0.5357 & -0.1948 & 0.3239 & 0.0055 & -0.3184 & 0.0165 \\
Corrected L2 & 0.8245 & 0.4852 & -0.3393 & 0.3365 & 0.0051 & -0.3314 & 0.0000 \\
\hline
\end{tabular*}
\end{table*}
...
\begin{revisionblock}
The experimental results indicate that descriptor performance varies across operational conditions and representation-learning strategies. The Fine-Tuned VGG-16 showed its strongest relative behavior under stable-viewpoint conditions, whereas ResNet showed stronger performance under opposite-traversal conditions in the evaluated sequences. These condition-specific differences are empirical observations and are not attributed here to a single internal architectural mechanism. Architecturally, residual connections are a defining property of ResNet \cite{He2016ResNet}, while VGG-16 uses a stacked convolutional hierarchy \cite{Simonyan2015VGG}; these properties provide interpretive context, but the present experiments do not establish either property as the cause of the measured performance differences.

A new internal representation-level comparison (e.g., layer-wise activations, t-SNE, or Grad-CAM) between ResNet-101 and the Fine-Tuned VGG-16 used for the reported experiments could not be reproduced faithfully because the original trained Fine-Tuned VGG-16 checkpoint is no longer available, and no exactly alignable frozen outputs from that model were recovered. Replacing it with an ImageNet-only or newly retrained VGG-16 would change the model underlying the historical results. We therefore treat architecture-based explanations as hypotheses rather than experimentally established causes. This limitation concerns reproducibility of new internal analyses and does not alter the identity or terminology of the Fine-Tuned VGG-16 implementation used in the original experiments.
\end{revisionblock}
...
% R2-M2 preprocessing-ablation interpretation (candidate patch)
\begin{revisionblock}
These results do not support a universal preference for either preprocessing
mode. Full-image processing was clearly advantageous on TUM19 and KITTI 05,
whereas KITTI 08 showed no clear held-out ranking advantage for either mode.
The effect of image preprocessing should therefore be regarded as
dataset-dependent. Accordingly, the left/right splitting strategy is retained
as a historically motivated design choice rather than presented as a
universally beneficial preprocessing rule.
\end{revisionblock}
...
\begin{revisionblock}
The experimental scope of this study is limited to KITTI Groups 5 and 8 and the evaluated TUM sequence. These sequences were used to expose complementary conditions represented in the experiments, including relatively stable viewpoints, opposite traversal directions, and camera rotation. Accordingly, the architecture, descriptor, and preprocessing trends reported here should be interpreted within these benchmark conditions rather than as universal performance claims across all Visual SLAM environments.
\end{revisionblock}
\begin{revisionblock}
The experimental results indicate that descriptor performance varies across operational conditions and representation-learning strategies. Among the evaluated architectures, the Adapted and Fine-Tuned VGG-16 achieved the best performance under relatively stable viewpoint conditions and showed improved descriptor discriminability across the evaluated scenarios. ResNet exhibited improved performance under opposite-traversal conditions in the evaluated sequence; this is reported as a condition-specific empirical difference rather than evidence of a particular internal architectural mechanism. The experiments also indicate that camera rotations reduced descriptor consistency, increasing FP and FN rates during descriptor matching and decreasing overall detection reliability.
\end{revisionblock}
...
\begin{revisionblock}
From a practitioner perspective, the results support condition-dependent model and pipeline selection rather than a single overall ranking. In the historical multi-architecture comparison, the Fine-Tuned VGG-16 showed the strongest performance under relatively stable viewpoints, whereas ResNet showed the stronger result under opposite-traversal conditions; substantial camera rotation should still be treated as a challenging deployment condition. These architecture-specific observations refer to the historical multi-architecture comparison and should not be numerically merged with the separate descriptor-level baseline audit. Descriptor-level discrimination should be evaluated separately from temporal verification, and Accuracy should be complemented by F1-score and threshold-independent ranking metrics for the imbalanced Loop Closure Detection setting. The preprocessing ablation also indicates that left/right splitting should not be assumed beneficial across datasets. For implementation trade-offs on the stated hardware, DBoW2 favors CPU extraction latency, NetVLAD-512 favors descriptor compactness and exhaustive matching speed, and ResNet-101 occupies an intermediate extraction regime with a larger descriptor. These practitioner-oriented recommendations are limited to the evaluated KITTI and TUM conditions.
\end{revisionblock}
